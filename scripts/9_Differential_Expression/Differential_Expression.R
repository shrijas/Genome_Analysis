...
cs15_fore <- read.table("/home/karthik/Desktop/Shrija/cs15_forelimb_htseqresults.txt", head=FALSE)
cs15_hind <- read.table("/home/karthik/Desktop/Shrija/cs15_hindlimb_htseqresults.txt", head=FALSE)
cs16_fore <- read.table("/home/karthik/Desktop/Shrija/cs16_forelimb_htseqresults.txt", head=FALSE)
cs16_hind <- read.table("/home/karthik/Desktop/Shrija/cs16_hindlimb_htseqresults.txt", head=FALSE)
cs17_fore <- read.table("/home/karthik/Desktop/Shrija/cs17_forelimb_htseqresults.txt", head=FALSE)
...
genes_index <- read.csv2("GeneNames.csv", header = TRUE, sep=",")
# check if the names are the same (in the same order)
print(genes_index$Pred_Name == row.names(countData))
# add column with gene names
countData <- cbind(countData,genes_index$blast_name)
countData<- countData[countData$`genes_index$blast_name` != "-",]
countData<- countData[countData$`genes_index$blast_name` != 0,]
...
cts = countData
coldata = read.table("template.csv")
...
gene2 = which(rownames(res) == "UBE2B")
p<- plotCounts(dds, gene2, intgroup=c("condition","type"), returnData = TRUE)
ggplot(p, aes(x=condition, y=count, color=type)) + 
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10(breaks=c(10,100, 1000,5000))+
...
gene2 = which(rownames(res) == "SMAD5")
p<- plotCounts(dds, gene2, intgroup=c("condition","type"), returnData = TRUE)
ggplot(p, aes(x=condition, y=count, color=type)) + 
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10(breaks=c(10,100, 1000,5000))+
